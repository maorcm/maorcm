...
class MYSQL_DB
{
    ...
    function trainUnit($vid, $unit, $amt, $pop, $each, $commence, $mode)
    {
        ...
        if (!$mode) {
            ...
            if (count($queued) > 0) {
                ...
                $timestamp = $commence;
                $endat = (($queued[count($queued) - 1]['endat'] > $_SERVER['REQUEST_TIME']) ? $queued[count($queued) - 1]['endat'] : $_SERVER['REQUEST_TIME']) + ($each * $amt / 1000);
            } else {
			...
			if ($queued[count($queued) - 1]['unit'] == $unit) {
                ...
                $q = "UPDATE " . TB_PREFIX . "training SET amt = amt + $amt,endat = $endat,timestamp = timestamp + $time WHERE id = " . $queued[count($queued) - 1]['id'] . "";
            } else {
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "wdata USE INDEX (PRIMARY) left JOIN " . TB_PREFIX . "odata ON " . TB_PREFIX . "odata.wref = " . TB_PREFIX . "wdata.id where " . TB_PREFIX . "wdata.id = $id";
        $result = mysql_query($q, $this->connection);
        return mysql_fetch_array($result);
    }
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "wdata USE INDEX (PRIMARY) left JOIN " . TB_PREFIX . "vdata ON " . TB_PREFIX . "vdata.wref = " . TB_PREFIX . "wdata.id where " . TB_PREFIX . "wdata.id = $id";
        $result = mysql_query($q, $this->connection);
        return mysql_fetch_array($result);
    }
	...
	function getMovement($type, $village, $mode) {
		...
		switch($type) {
			...
			case 2:
				...
				break;
			case 3:
				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where " . TB_PREFIX . "movement." . $where . " = $village and spy = 0 and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0 and " . TB_PREFIX . "movement.sort_type = 3 ORDER BY endtime ASC";
				break;
			case 4:
			...
			case 9:
				...
				break;
			case 34:
				$q = "SELECT * FROM " . TB_PREFIX . "movement, " . TB_PREFIX . "attacks where (" . TB_PREFIX . "movement." . $where . " = $village and " . TB_PREFIX . "movement.ref = " . TB_PREFIX . "attacks.id and " . TB_PREFIX . "movement.proc = 0) and spy = 0 and (" . TB_PREFIX . "movement.sort_type = 3 or  " . TB_PREFIX ."movement.sort_type = 4) ORDER BY endtime ASC";
				break;
		}
		$result = mysql_query($q, $this->connection);
		$array = $this->mysql_fetch_all($result);
    ...
    {
        if ($commence == 0) $commence = $_SERVER['REQUEST_TIME'];
    ...
    {
        $q = "INSERT INTO `" . TB_PREFIX . "artefacts` (`vref`, `owner`, `type`, `size`, `conquered`, `name`, `desc`, `effecttype`, `effect`, `aoe`, `img`) VALUES ('$vref', '$owner', '$type', '$size', '" . $_SERVER['REQUEST_TIME'] . "', '$name', '$desc', '$effecttype', '$effect', '$aoe', '$img')";
        return mysql_query($q, $this->connection);
    }
    ...
    {
        $hash = md5("$uid" . $_SERVER['REQUEST_TIME']);
    ...
    public function hasBeginnerProtection($vid)
    {
        ...
        $dbarray = mysql_fetch_array($result);
        if (!empty($dbarray)) {
            if ($_SERVER['REQUEST_TIME'] < $dbarray[0]) {
                return true;
            } else {
                return false;
            }
    ...
    {
        $time = $_SERVER['REQUEST_TIME'];
        $q = "INSERT INTO " . TB_PREFIX . "mdata values (0,$client,$owner,'$topic',\"$message\",1,0,$send,$time,0,0,$alliance,$player,$coor,$report)";
        return mysql_query($q, $this->connection);
    }
    ...
    function removeOases($wref)
    {
        ...
        $r2 = mysql_query($q, $this->connection)or die(mysql_error());
        return ($r && $r2);
    }
    ...
    {
        $time = $_SERVER['REQUEST_TIME'] + AUCTIONTIME;
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "hero WHERE dead = 0 and uid = $uid LIMIT 0, 1";
        $result = mysql_query($q, $this->connection);
        return mysql_fetch_assoc($result);
    }
    ...
    function getVFH($uid)
    {
        $q = "SELECT * FROM " . TB_PREFIX . "vdata WHERE owner = $uid and capital = 1";
        $result = mysql_query($q, $this->connection);
        $dbarray = mysql_fetch_array($result);
        return $dbarray['wref'];
    }
    ...
    {
        $approved = 1;
        $approvedate = $_SERVER['REQUEST_TIME'];
    ...
    {
        $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `vref`=' . $vref . ' AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600));
        return $this->query_return($q);
    }

    function getAccountActiveArte($owner)
    {
        $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `owner`=' . $owner . ' AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600));
        return $this->query_return($q);
    }
    ...
    function getArteEffectByType($wref, $type)
    {
        ...
        if (!empty($vinfo) && isset($vinfo['owner'])) {
            $owner = $vinfo['owner'];
            $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `owner`=' . $owner . ' AND `effecttype`=' . $type . ' AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600)) . ' ORDER BY `conquered` DESC';
            $result = $this->query_return($q);
            if (!empty($result) && count($result) > 0) {
                $i = 0;
                foreach ($result as $r) {
    ...
    {
        $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `type`=3 AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600)) . ' AND lastupdate<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600));
        $result = $this->query_return($q);
        if (!empty($result) && count($result) > 0) {
            foreach ($result as $r) {
                $effecttype = rand(3, 9);
    ...
    function getArtEffBP($wref)
    {
        ...
        $vinfo = $this->getVillage($wref);
        $owner = $vinfo['owner'];
        $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `owner`=' . $owner . ' AND `effecttype`=11 AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600)) . ' ORDER BY `conquered` DESC';
        $result = $this->query_return($q);
        if (!empty($result) && count($result) > 0) {
            return $artEff = 1;
        } else {
    ...
    function getArtEffAllyBP($uid)
    {
        ...
        if (!empty($diplos) && count($diplos) > 0) {
            ...
            if (!empty($mate) && count($mate) > 0) {
                ...
                $matestr = implode(',', $ml);
                $q = 'SELECT * FROM ' . TB_PREFIX . 'artefacts WHERE `owner` IN (' . $matestr . ') AND `effecttype`=11 AND `status`=1 AND `conquered`<=' . ($_SERVER['REQUEST_TIME'] - max(86400 / SPEED, 600)) . ' ORDER BY `conquered` DESC';
                $result = $this->query_return($q);
                if (!empty($result) && count($result) > 0) {
                    return $artEff = 1;
                }
    ...
    {
        $q = "INSERT INTO " . TB_PREFIX . "emailinvite (`uid`,`invemail`,`time`) VALUES ('$uid', '$invemail', '" . $_SERVER['REQUEST_TIME'] . "')";
        if (mysql_query($q, $this->connection)) {
            return mysql_insert_id($this->connection);
        } else {
            return false;
    ...
    {
        $q = "SELECT * from " . TB_PREFIX . "users WHERE email = " . $mail . " LIMIT 0,1";
        $result = mysql_query($q, $this->connection);
        $dbarray = mysql_fetch_assoc($result);
        return $dbarray;
    }
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "users WHERE id = $id  LIMIT 0,1";
        $result = mysql_query($q, $this->connection);
        $dbarray = mysql_fetch_assoc($result);
        return $dbarray;
    }
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "users WHERE id = '" . $uid . "'  LIMIT 0,1";
        $result = mysql_query($q, $this->connection);
        $dbarray = mysql_fetch_assoc($result);
        return $dbarray;
    }
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "users WHERE username = '" . $name . "'  LIMIT 0,1";
        $result = mysql_query($q, $this->connection);
        $dbarray = mysql_fetch_assoc($result);
        return $dbarray;
    }
    ...
    {
        $q = "SELECT * FROM " . TB_PREFIX . "stats where owner = " . $uid . "";
        $t = 0;
        if ($inf == 'rank') {
            $res = mysql_query($q);
